...
import { CreateHospitalRequest, HospitalResponse, UpdateHospitalRequest } from "../../models";
...
export const HospitalForm: FC<HospitalFormProps> = ({ isEditMode }) => {
	...
	useEffect(() => {
		...
		const selectedHospital = hospitals.find((hospital) => hospital.id === Number(id));
	...
	return (
		<AuthorizedRoute>
			...
			>
				{!isSubmitting ? (
					<form onSubmit={handleSubmit(isEditMode ? handleUpdateHospital : handleAddHospital)}>
						<Input
							name="name"
							label="Hospital Name"
							register={register}
						...
						<Input
							...
							disabled={isEditMode}
						/>
						<LabelContainer name="ownership" label="Ownership" error={errors.image?.message}>
							<div className="hospitalFilterContainer ownershipContainer ">
								<select {...register("ownership")} className="hospitalFilter" disabled={isEditMode}>
						...
						<LabelContainer name="about" label="About" error={errors.about?.message}>
							<textarea
								id="about"
